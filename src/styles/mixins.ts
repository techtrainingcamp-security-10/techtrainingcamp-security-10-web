...
export const typography = (level: string) => {
  const levels: any = {
    'h0': css`
      font-size: 30px;
      line-height: ${38 / 30};
...
export const centerBlock = (width?: any) => css`
  display: block;
  margin-left: auto;
  margin-right: auto;
  ${width &&
...
export const size = (width: any, height = width) => css`
  width: ${width};
  height: ${height};
`;
...
export const logoTitle = (level: string) => css`
  ${reset()};
  ${typography(level)};
  ${flexVerticalCenter()};
  color: #172d72;
